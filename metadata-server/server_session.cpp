...
server_session::handle_registration_request (const system::error_code& err, size_t n, struct protocol_packet *request)
{
  ...
  if (!err) {
    ...
    tcp::endpoint endpoint_ (ip::address_v4 (registration_req->addr), registration_req->port);
...
server_session::handle_fetch_resolution_request (const system::error_code& err, size_t n, struct protocol_packet *request)
{
  ...
  if (!err) {
    ...
    map<uint32_t, vector<tcp::endpoint> >::iterator iter = mds->filenames.find (fetch_resolution_req->hash_code);
...
void
server_session::write_resolution_response (vector<tcp::endpoint> &endpoints)
{
  struct protocol_packet *response = (struct protocol_packet *) malloc (sizeof(struct protocol_packet));
...
void
server_session::random_replication (uint32_t hash_code, u_int8_t replicas, vector<tcp::endpoint> &endpoints)
{
