...
public:
   ...
   */
  encoding_state *
  init_state (unsigned char *blob_id, unsigned int blob_size, unsigned char *blob);
  /*
...
inline
encoder::encoder () : generator (rd()), seeder (rd())
{
}
...
{
  cout << "encoder: copy constructor NOT implemented yet" << endl;
}
...
encoder::init_state (unsigned char *blob_id, unsigned int blob_size, unsigned char *blob)
{
  map<unsigned int, robust_soliton_distribution *>::iterator soliton_distr_iter;
  map<unsigned int, boost::random::uniform_int_distribution<unsigned int> *>::iterator uniform_distr_iter;
  ...
  return new encoding_state (blob_id, blob_size, blob, number_of_fragments, size_of_last_fragment, soliton_distribution, uniform_distribution);
}

inline symbol *
encoder::encode_next (encoding_state *enc_state)
{
  set<unsigned int> duplicate_neighbours_guard_set;
  ...
  for (unsigned int i = 0; i < new_symbol->degree; i++) {
	...
	if (neighbour == enc_state->number_of_fragments) {
	  ...
	  memcpy (new_symbol->symbol_data, enc_state->blob + ((neighbour - 1) * SYMBOL_SIZE), enc_state->size_of_last_fragment);
	} else {
	  memcpy (new_symbol->symbol_data, enc_state->blob + ((neighbour - 1) * SYMBOL_SIZE), SYMBOL_SIZE);
	}
	///////////////////////////////////////////////////////////////////////////////////////////
      } else {
	///////////////////////////////////////////XOR/////////////////////////////////////////////
	if (neighbour == enc_state->number_of_fragments) {
	  trevi_chunk_xor_128 (enc_state->blob + (neighbour - 1) * SYMBOL_SIZE, new_symbol->symbol_data, enc_state->size_of_last_fragment);
	} else {
	  trevi_chunk_xor_128 (enc_state->blob + (neighbour - 1) * SYMBOL_SIZE, new_symbol->symbol_data, SYMBOL_SIZE);
	}
	///////////////////////////////////////////////////////////////////////////////////////////
      }
    } else {
