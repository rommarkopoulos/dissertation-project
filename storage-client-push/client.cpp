...
client::write_start_storage_request (const boost::system::error_code& err, std::size_t n, struct push_protocol_packet *request, udp::endpoint ds_endpoint, size_t encoded_hash)
{
  ...
  socket_udp.async_send_to (
      ...
      boost::bind (&client::start_storage_request_written, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, request, ds_endpoint, encoded_hash));
...
//  cout << "handle_request()" << endl;
  ...
  if (!err) {
	...
	if (decoding_iter != decodings.end ()) {
	  ...
	  ///////////////////////////////////////////////HASH OF BLOB///////////////////////////////////////////////
	  string blob_to_encode_str ((const char *) decoding_iter->second->blob, BLOB_SIZE);
	  boost::hash<std::string> blob_to_encode_str_hash;
	  std::size_t encoded_hash = blob_to_encode_str_hash (blob_to_encode_str);
	  string enc_hash = sizeToString (encoded_hash);
	  //////////////////////////////////////////////////////////////////////////////////////////////////////////
	  greenColor ("client");
	  cout << "Data of blob " << request->push_payload.symbol_data.hash_code << " was successfully fetched" << endl;
	  greenColor ("client");
	  cout << "Hash of data = ";
	  greenColor (enc_hash);
	  cout << endl;
