...
public:
  ...
  void
  storage_dataservers_resolved(const system::error_code& err, vector<tcp::endpoint> &endpoints, u_int32_t hash_code, char* data, uint32_t length, storage_callback storage_cb);

  void
  fetch_dataservers_resolved (const system::error_code& err, vector<tcp::endpoint> &endpoints, u_int32_t hash_code, fetch_callback fetch_cb);
  ...
  /* map of client sessions (key is ip:port) */
  map<tcp::endpoint, client_session_ptr> data_sessions;
